...
// cleanCGI function to remove hyphens before matching
// cleanCGI function to remove hyphens before matching
func cleanCGI(raw string) string {
	// Remove hyphens from the input
	raw = strings.ReplaceAll(raw, "-", "")
	if len(raw) > 6 {
		return raw[6:]  // Remove the first 6 digits if needed
	}
	return raw
}
...
func normalizeAirtel(src, dst, crime, tsp string) error {
	...
	for {
		...
		key := cleanCGI(rec[col["cgi"]]) // Clean hyphens here before mapping
