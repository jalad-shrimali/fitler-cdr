...
var cdrRE = regexp.MustCompile(`(?i)input\s*value[^0-9]*([0-9]{8,15})`)
func extractCDR(line string) string {
	if m := cdrRE.FindStringSubmatch(line); len(m) > 1 { return m[1] }
	return ""
}
...
/* init: load meta + open DB */
func init() {
	for _, r := range loadCSV(filepath.Join("jio", "data", "Headers.csv")) {
		if len(r) >= 2 {
			alias2canon[norm(r[0])] = r[1]
			alias2canon[norm(r[1])] = r[0]
		}
	}
	for _, r := range loadCSV(filepath.Join("jio", "data", "Call_types.csv")) {
		if len(r) > 0 { callAlias[norm(r[0])] = struct{}{} }
	}
	rows := loadCSV(filepath.Join("jio", "data", "LRN.csv"))
	if len(rows) > 1 {
		h := rows[0]
	...
	cellDB, err = sql.Open("sqlite3",
		fmt.Sprintf("file:%s?mode=ro", filepath.Join("jio", "data", "testnewcellids.db")))
...
func UploadAndNormalizeCSV(w http.ResponseWriter, r *http.Request) {
	...
	if norm(r.FormValue("tsp_type")) != "jio" { http.Error(w, "Only Jio", 400); return }
...
/* ── core processor ── */
func processJio(src, crime string) (string, error) {
	...
	first, _ := r.Read()
	if cdr == "" {
		if idx := colIdx(header, "input value"); idx != -1 && idx < len(first) {
	...
	dstIx := func(name string) int { for i, h := range targetHeader { if h == name { return i } }; return -1 }
	src2dst := map[int]int{}
	var bCols []int
	...
	for i, h := range header {
		...
		if alias2canon[n] == "B Party" {
			if d := dstIx("B Party"); d != -1 {
				src2dst[i] = d; bCols = append(bCols, i)
			}
			...
			if d := dstIx("Call Type"); d != -1 {
				src2dst[i] = d
			}
		}
	}
	/* minimal hard-maps (skip “A Party”) */
	hmap := func(key, canon string) {
		if i := colIdx(header, key); i != -1 {
			if d := dstIx(canon); d != -1 { src2dst[i] = d }
		}
	}
	hmap("call date", "Date");        hmap("call time", "Time")
	hmap("call duration", "Duration"); hmap("called party telephone number", "B Party")
	hmap("first cgi", "First Cell ID"); hmap("last cgi", "Last Cell ID")
	hmap("imei", "IMEI");             hmap("imsi", "IMSI")
	hmap("roaming circle name", "Roaming"); hmap("lrn called no", "LRN")
	hmap("call forward", "CallForward");    hmap("service type", "Type")
	...
	process := func(rec []string) {
		...
		for s, d := range src2dst {
			...
			if d == dstIx("B Party") {
				if dig := last10(val); dig != "" && dig != cdr10 { val = dig }
			}
			row[d] = val
		}
		row[dstIx("CdrNo")] = cdr
		...
		/* fallback B-Party using calling/called */
		if row[dstIx("B Party")] == "" {
			iCall := colIdx(header, "calling party telephone number")
			iCalled := colIdx(header, "called party telephone number")
			call, called := "", ""
			if iCall != -1 && iCall < len(rec) { call = rec[iCall] }
			if iCalled != -1 && iCalled < len(rec) { called = rec[iCalled] }
			switch {
			case last10(call) == cdr10: row[dstIx("B Party")] = called
			case last10(called) == cdr10: row[dstIx("B Party")] = call
			default: row[dstIx("B Party")] = called
			}
		}
		...
		/* party agg */
		b := row[dstIx("B Party")]; if b == "" { b = "(blank)" }
		...
		updateDT(row[dstIx("Date")], row[dstIx("Time")], &pa.Fd, &pa.Ft, &pa.Ld, &pa.Lt)
		...
		if fid != "" {
			...
			updateDT(row[dstIx("Date")], row[dstIx("Time")], &ca.Fd, &ca.Ft, &ca.Ld, &ca.Lt)
		}
	}
	...
	out := filepath.Join("filtered", cdr+"_jio_all_reports.xlsx")
